...
async function sendTelegramMessage(text) {
  ...
  if (!res.ok) {
    ...
    console.error('[error] Telegram send failed:', res.status, body);
  }
}
...
async function tick(seen, isFirstRun) {
  ...
  if (newPairs.length === 0) {
    console.log(`[${new Date().toISOString()}] No new pairs (${pairs.length} total).`);
    return;
  }

  console.log(`[${new Date().toISOString()}] ${newPairs.length} new pair(s) found!`);
